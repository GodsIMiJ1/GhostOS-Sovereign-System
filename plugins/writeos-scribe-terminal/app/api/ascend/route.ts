...
export async function POST(request: NextRequest) {
  try {
    ...
    const processedResult = await processContentWithAI(content, action, context, aiService);
...
async function processContentWithAI(content: string, action: string, context: string | undefined, aiService: AIService) {
  ...
  try {
    switch (action) {
      case 'enhance':
        processedContent = await aiService.enhance(content, context);
        suggestions = [
      ...
      case 'summarize':
        processedContent = await aiService.summarize(content, context);
        suggestions = [
          'Key points extracted using AI analysis',
          'Review summary for completeness',
          'Consider expanding important details'
        ];
        break;
      ...
      case 'analyze':
        const analysis = await aiService.analyze(content, context);
        processedContent = generateAnalysisReport(analysis);
    ...
    const fallbackResult = await processContent(content, action, context);
...
function generateAnalysisReport(analysis: { readabilityScore: number; sentiment: string; keyTopics: string[]; suggestions: string[]; wordCount: number; estimatedReadingTime: number }): string {
  return `# AI Content Analysis Report
...
- **Clarity:** ${analysis.readabilityScore > 80 ? 'Excellent' : analysis.readabilityScore > 60 ? 'Good' : 'Needs Improvement'}
- **Engagement:** ${analysis.sentiment === 'positive' ? 'High' : analysis.sentiment === 'neutral' ? 'Moderate' : 'Low'}
- **Structure:** ${analysis.keyTopics.length > 3 ? 'Well-organized' : 'Could be more detailed'}`;
}

// Keep original processing as fallback
async function processContent(content: string, action: string, _context?: string) {
  const startTime = Date.now();
