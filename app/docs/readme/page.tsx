...
export default function ReadmePage() {
  const [content, setContent] = useState('# 🔥 Loading Sovereign Documentation...\n\nPlease wait while the Empire\'s sacred README is being retrieved...');
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    // Fetch README content from a simple API route
    fetch('/api/readme')
      ...
      .catch(error => {
        ...
        setContent('# 🔥 README.md Not Found\n\nThe sovereign documentation could not be loaded.');
        setIsLoading(false);
      });
  }, []);

  return (
    <div className="min-h-screen max-h-screen overflow-hidden bg-gradient-to-br from-black via-zinc-900 to-black flex flex-col">
      {/* Header - Fixed */}
      <div className="bg-zinc-900/50 border-b border-flame/20 backdrop-blur-sm flex-shrink-0">
        <div className="max-w-6xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {isLoading && (
                <span className="px-3 py-1 bg-blue-500/20 border border-blue-500/30 rounded-full text-blue-400 text-xs font-medium animate-pulse">
                  📡 LOADING
                </span>
              )}
            </div>
      ...
      {/* Footer - Fixed */}
      <div className="bg-zinc-900/50 border-t border-flame/20 backdrop-blur-sm flex-shrink-0">
        <div className="max-w-6xl mx-auto px-6 py-4">
