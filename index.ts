...
import { GhostAugmenth } from './plugins/ghost_augmenth/GhostAugmenth.js';
import { GhostPluginAPI } from './api/GhostPluginAPI.js';
...
export class GhostOS {
  ...
  private pluginAPI: GhostPluginAPI;
  ...
  constructor() {
    ...
    this.pluginManager = new PluginManager(this.relay);
    this.pluginAPI = new GhostPluginAPI(this.pluginManager, this.relay);
  ...
  async shutdown(): Promise<void> {
    ...
    try {
      // Stop API server
      await this.pluginAPI.stop();

      // Stop all plugins
      const runningPlugins = this.pluginManager.getRunningPlugins();
      for (const plugin of runningPlugins) {
        this.log(`Stopping plugin ${plugin.manifest.name}...`, 'info');
        await this.pluginManager.stopPlugin(plugin.manifest.name);
      }
  ...
  getStatus(): any {
    ...
    return {
      ...
      plugins: {
        ...
        list: allPlugins.map(p => p.manifest.name)
      },
      relay: {
      ...
      api: {
        port: 3000,
        endpoints: ['/api/plugins', '/api/system']
      },
  ...
  private async initializePluginSystem(): Promise<void> {
    this.log('🔌 Initializing enhanced plugin system...', 'info');

    // Discover and load plugins
    await this.pluginManager.discoverPlugins();

    // Register ghost_augmenth plugin factory
    this.pluginManager.registerPluginFactory('ghost_augmenth', () => new GhostAugmenth());

    // Load ghost_augmenth plugin
    await this.pluginManager.loadPlugin('ghost_augmenth');

    // Start auto-start plugins
    const plugins = this.pluginManager.getAllPlugins();
    for (const plugin of plugins) {
      if (plugin.manifest.autoStart) {
        await this.pluginManager.startPlugin(plugin.manifest.name);
      }
    }

    this.log(`🔌 Plugin system initialized with ${plugins.length} plugins`, 'success');
  }

  private async startAPIServer(): Promise<void> {
    try {
      await this.pluginAPI.start();
      this.log('🌐 GhostPluginAPI server started on port 3000', 'success');
    } catch (error) {
