...
export class FlameCLI {
  ...
  private initializeCommands(): void {
    ...
    this.addCommand({
      ...
      usage: 'plugin <list|start|stop|restart|status> [pluginName]',
      handler: this.handlePlugin.bind(this)
    });

    this.addCommand({
      name: 'augmenth',
      description: 'Ghost_Augmenth plugin commands',
      usage: 'augmenth <deploy|reflect|status> [args...]',
      handler: this.handleAugmenth.bind(this)
    });
  ...
  private handlePlugin(args: string[]): void {
    if (!this.relay) {
      this.log('No OmniRelay connection', 'error');
      return;
    }
    ...
    if (args.length === 0) {
      this.log('Available plugin operations: list, start, stop, restart, status', 'info');
      return;
    }
    ...
    switch (operation) {
      case 'list':
        this.relay.route('plugin_list_request', {}, 'FlameCLI');
        this.log('Plugin list requested', 'info');
        break;
      case 'start':
        if (!pluginName) {
          this.log('Usage: plugin start <pluginName>', 'error');
          return;
        }
        this.relay.route('plugin_start_request', { pluginName }, 'FlameCLI');
        this.log(`Plugin start request sent: ${pluginName}`, 'success');
        break;
      case 'stop':
        if (!pluginName) {
          this.log('Usage: plugin stop <pluginName>', 'error');
          return;
        }
        this.relay.route('plugin_stop_request', { pluginName }, 'FlameCLI');
        this.log(`Plugin stop request sent: ${pluginName}`, 'success');
        break;
      case 'restart':
        if (!pluginName) {
          this.log('Usage: plugin restart <pluginName>', 'error');
          return;
        }
        this.relay.route('plugin_stop_request', { pluginName }, 'FlameCLI');
        setTimeout(() => {
        ...
        this.log(`Plugin restart request sent: ${pluginName}`, 'success');
        break;
      case 'status':
        if (!pluginName) {
          this.log('Usage: plugin status <pluginName>', 'error');
          return;
        }
        this.relay.route('plugin_status_request', { pluginName }, 'FlameCLI');
        this.log(`Plugin status request sent: ${pluginName}`, 'info');
        break;
      default:
        this.log(`Unknown plugin operation: ${operation}`, 'error');
    }
  }

  private handleAugmenth(args: string[]): void {
    if (!this.relay) {
      this.log('No OmniRelay connection', 'error');
      return;
    }
    ...
    if (args.length === 0) {
      this.log('Available augmenth operations: deploy, reflect, status', 'info');
      return;
    }
    ...
    switch (operation) {
      case 'deploy':
        ...
        if (!project) {
          this.log('Usage: augmenth deploy <project> [branch]', 'error');
          return;
        }
        this.relay.route('augment_deploy', { project, branch }, 'FlameCLI', 'ghost_augmenth');
        this.log(`🔥 Augment deployment initiated: ${project}:${branch}`, 'success');
        break;
      case 'reflect':
        const action = args[1];
        const description = args.slice(2).join(' ');
        if (!action || !description) {
          this.log('Usage: augmenth reflect <action> <description>', 'error');
          return;
        }
        this.relay.route('augment_reflect', {
          action,
          description,
          outcome: 'success',
          metadata: { source: 'FlameCLI' }
        }, 'FlameCLI', 'ghost_augmenth');
        this.log(`🔮 Reflection created: ${action} - ${description}`, 'success');
        break;
      case 'status':
        this.relay.route('ping', { timestamp: new Date().toISOString() }, 'FlameCLI', 'ghost_augmenth');
        this.log('🔥 Augmenth status requested', 'info');
        break;
      default:
        this.log(`Unknown augmenth operation: ${operation}`, 'error');
    }
  }
